...
class PollingClient extends EventEmitter {
    constructor({ pollingInterval = 500, tries = 5, logger } = {}) {
        super()
        this._polling = false
        this._tries = tries
        this._pollingInterval = parseInt(pollingInterval, 10)
    ...
    async start({ task, context, callback, runCondition, runCount, tries = this._tries } = {}) {
        ...
        this._polling = true
        this.emit('started')
        this._logger(`Polling Manager polling every [${this._pollingInterval}] ms`)
